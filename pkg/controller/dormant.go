...
func (c *Controller) waitUntilDeploymentsDeleted(db *api.Redis) error {
	log.Infof("waiting for deployments for Redis %v/%v to be deleted\n", db.Namespace, db.Name)
	return wait.PollImmediate(kutil.RetryInterval, kutil.GCTimeout, func() (bool, error) {
		if deploys, err := c.Client.AppsV1().Deployments(db.Namespace).List(
			context.TODO(),
			metav1.ListOptions{LabelSelector: labels.SelectorFromSet(db.OffshootSelectors()).String()}); err != nil && kerr.IsNotFound(err) || len(deploys.Items) == 0 {
			return true, nil
		}
		return false, nil
	})
...
//********
// wipeOutDatabase is a generic function to call from WipeOutDatabase and Redis pause method.
...
func (c *Controller) secretsUsedByPeers(meta metav1.ObjectMeta) (sets.String, error) {
	...
	dbList, err := c.rdLister.Redises(meta.Namespace).List(labels.Everything())
	if err != nil {
		return nil, err
	}
